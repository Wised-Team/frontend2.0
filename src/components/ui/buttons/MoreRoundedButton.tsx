...
export interface MoreRoundedButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement>, VariantProps<typeof buttonVariants> {
  text: string
}
...
const MoreRoundedButton: React.FC<MoreRoundedButtonProps> = ({
  ...
  return (
    <button className={cn(buttonVariants({ variant, size, className }))}>{props.text}</button>
  )
}
