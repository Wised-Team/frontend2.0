...
const Toggler: React.FC<TogglerProps> = ({
  ...
  const smallerCircle =
    size === 'sm' ? 'w-[1.4rem] h-[1.4rem]' : 'w-[1.8rem] h-[1.8rem]';
  return (
    <div
      className={cn(togglerVariants({ variant, size, className }))}
      onClick={() => handleToggle()}
    >
      <div
        className={`absolute h-[1.8rem] w-[1.8rem] rounded-full bg-white ${
          isActive ? 'translate-x-0' : 'translate-x-[200%]'
        }  ${smallerCircle} transition-all duration-200`}
      ></div>
    </div>
  );
};
